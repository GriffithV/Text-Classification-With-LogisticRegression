...
int main(){
	...
	printf("\n  Reading %s !",readSentFile(testsentl) ? "succeeded" : "failed" );
	double *w=(double*)calloc(dict->wordCounter,sizeof(double));
	printf("\nPlease enter initial w : ");
	scanf("%lf",&w[0]);
	for(i=1;i<dict->wordCounter;i++)	w[i]=w[0];
	//FILE* file = fopen("w for gD.csv","w");// TO COLLECT DATAS FOR GRAPHS
	//fprintf(file,"\n");
	
	//char *testsentence="Martial arts encompass various disciplines, such as karate, judo, taekwondo, and kung fu, focusing on self-defense and discipline.";
	//double *test=(double*)calloc(dict->wordCounter,sizeof(double));
	//createHotVector(dict,testsentence,test);
	clock_t start, end;
...
bool readDictFile(Dictionary* dict){
	...
	while ( fscanf(f_ptr , "%s" , buffer) != EOF ){ 
		...
		if ( !isExist(buffer,dict) && strlen(buffer) > 0){
			strcpy(dict->wordList[dict->wordCounter],buffer); // if length of buffer is still > 0 then push it to the dictionary
			dict->wordCounter++;
...
bool readSentFile(SentenceList* sentl){
	...
	while ( !feof(f_ptr) ){
		ch = fgetc(f_ptr);
		if ( ch != '.' && ch != '?' && ch != '!') { // add extra punctions like ! ? ... etc.
			if (i == 0 && ch == ' ');
			else sentence[i++] = ch;   
		}
		else{
...
void trimWord(char* w){
	...
	if (strlen(w) == 1){
		...
		for(i = 0; i<strlen(w) ;i++){
			if( isdigit(w[i]) ){
				w[0] = '\0'; // NULLing the first element in case of the word consisting of numbers (digits)	
				break;
			}
			if(ispunct(w[i])){
				if( w[i] != '-'){
			...
			// if current char is a '-' and char before & after it is an alphabetical char then keep it
			// otherwise just delete it by moving chars from right to left until that char's index.
			else w[i] = tolower(w[i]); // if current char is an alphabetical char just make it lowercase
		}
}
...
void gradientDescent(double *W,SentenceList *sentl,Dictionary *dict){
    ...
    for (iter = 0; iter < ITER; iter++) {
        ...
        for(j=0;j<160;j++){
	      ...
	      sentence_hotVector = (double*)calloc(dict->wordCounter,sizeof(double));
          createHotVector(dict,sentl->sentenceList[j],sentence_hotVector);

        for (i = 0; i < dict->wordCounter; i++) {
			wx=W[i]*sentence_hotVector[i];
            grad=(double)(1.00/dict->wordCounter)*(-2*(desired-tanh(wx))*dtanh(wx)*sentence_hotVector[i]);
			W[i] =W[i] - LR*grad;
			wx=0;
        }
